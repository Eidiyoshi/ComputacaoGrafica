  ...
  begin
    ...
    begin
      ...
      if((cod1 = 0)and(cod2 = 0)) then Image1.Canvas.Line(x1,y1,x2,y2); // dois pontos dentro do quadrado
        ...
        if(m = 0) then m := 0.1;
        // descobrir limites
        yLim := trunc(m*(xLeft - x1)) + y1;
        limX := xLeft;
        if( not ((yBottom <= yLim) and (yLim <= yTop))) then
        begin
          limX := xRight;
          yLim := trunc(m*(xRight - x1)) + y1;
        end;

        xLim := trunc((yTop-y1)/m) + x1;
        limY := yTop;
        if( not ((xLeft <= xLim) and (xLim <= xRight))) then
        begin
          limY := yBottom;
          xLim := trunc((yBottom - y1)/m) + x1;
        end;

        if((x1>xLeft)and(x1<xRight)and(y1>yTop)and(y1<yBottom)) then Image1.Canvas.Line(x1,y1,xLim,limY);
        if((x2>xLeft)and(x2<xRight)and(y2>yTop)and(y2<yBottom)) then Image1.Canvas.Line(x2,y2,xLim,limY);

        Image1.Canvas.Pen.Color := clred;
        Image1.Canvas.Line(limX,yLim,xLim,limY);
